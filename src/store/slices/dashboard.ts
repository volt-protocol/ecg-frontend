...
const getTVL = async (marketId: number, chainId: number) => {
  const res: AxiosResponse<any, any> = await axios.get(getApiBaseUrl(chainId) + `/history/TVL?marketId=${marketId}`);
...
const getAprData = async (marketId: number, chainId: number) => {
  const url = getApiBaseUrl(chainId) + `/history/APR?marketId=${marketId}`;
  const data = await HttpGet<HistoricalAprData>(url);
  return data;
};

const GetLoanBorrow = async (marketId: number, chainId: number) => {
  const url = getApiBaseUrl(chainId) + `/history/LoanBorrow?marketId=${marketId}`;
  const data = await HttpGet<HistoricalLoanBorrow>(url);
  return data;
};
...
export const createDashboardSlice: StateCreator<DashboardSlice> = (set, get) => ({
  ...
  addLastVotes: (userAddress, array) => {
    ...
    if (!user) {
      set(() => ({
        userData: [{ address: userAddress, lastVotes: [...array], lastUpdated: Date.now() }, ...get().userData]
      }));
    } else {
      set(() => ({
        userData: [
  ...
  addLastMints: (userAddress, array) => {
    ...
    if (!user) {
      set(() => ({
        userData: [...get().userData, { address: userAddress, lastMints: [...array], lastUpdated: Date.now() }]
      }));
    } else {
      set(() => ({
        userData: [
  ...
  addUserLoans: (userAddress, array) => {
    ...
    if (!user) {
      set(() => ({
        userData: [{ address: userAddress, loans: [...array], lastUpdated: Date.now() }, ...get().userData]
      }));
    } else {
      set(() => ({
        userData: [
