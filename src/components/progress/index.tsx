const Progress = (props: { value: number; width?: string; max?: number; useColors: boolean }) => {
  const { value, width, useColors, max } = props;

  const getColor = (value) => {
    if (!useColors) return 'bg-brand-400 dark:bg-brand-400';
  ...
  return (
    <div className={`h-[10px] ${width ? width : 'w-full'} rounded-full bg-gray-200 dark:bg-navy-700`}>
      <div
