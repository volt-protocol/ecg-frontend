  ...
  return (
    <div style={{ width: `${width}px`, height: `${height}px` }} className={`flex h-8`}>
      <div
        ...
        className={clsx(forVotes == 100 ? 'rounded-md' : 'rounded-l-md', `w-1/3  bg-green-400`)}
      ></div>
      <div
        style={{ width: `${(abstainVotes / 100) * width}px` }}
        className={clsx(abstainVotes == 100 && 'rounded-md', `w-1/3  bg-gray-400`)}
      ></div>
      <div
        style={{ width: `${(againstVotes / 100) * width}px` }}
        className={clsx(againstVotes == 100 ? 'rounded-md' : 'rounded-r-md', `w-1/3  bg-red-400`)}
      ></div>
    </div>
  );
}
